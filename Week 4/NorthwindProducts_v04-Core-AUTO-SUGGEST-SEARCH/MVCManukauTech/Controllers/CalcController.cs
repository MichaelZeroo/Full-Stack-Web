...
//20180226 JPC JSON component - if this gives errors, it may need downloading with "NuGet"
...
namespace MVCManukauTech.Controllers
{
    public class CalcController : Controller
    {
        ...
        public IActionResult Mean(string csv)
        {
            ...
            string output = "<span style='font-size:30pt'>The mean is <span style='font-weight:bold;color:red'>" + mean.ToString() + "</span></span>";
        ...
        public string Result(string j)
        {
            ...
            double count = 0;
            double mean = 0;
            var results = JsonConvert.DeserializeObject<List<ResultViewModel>>(j);

            for(var i = 0; i< results.Count; i++)
            {
                if(results[i].TeamId == 2)
                {
                    total += results[i].Wins;
                    count += 1;
                }
            }
            mean = total / count;
